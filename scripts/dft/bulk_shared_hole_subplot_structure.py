...
atoms = 435
run = 'ai_forces'
folder_2 = 'E:/University/PhD/Programming/dft_ml_md/output/surfin/dft-md/data/philipp-share/bulk/hole/single-points/analysis'
value = 'Spin'
...
rows, cols = 5, 2
run_count = 0
fig_struct1, ax_struct1 = plt.subplots(rows, cols, sharex='col', sharey='row', figsize=(8, 9))
for row in range(rows):
    for col in range(cols):
        ...
        for i in range(len(atoms_fe)):
            ...
            ax_struct1[row, col].plot(time_plot[run_count], metric[run_count], 'k.')
        ax_struct1[row, col].set_ylim(ylim_2)
        ax_struct1[row, col].set_xlim([0-0.2, len(universe.trajectory) * timestep + 0.2])
        run_count = run_count + 1
ax_struct1[4, 0].set_xlabel('Time / fs', fontsize=11)
ax_struct1[4, 1].set_xlabel('Time / fs', fontsize=11)
ax_struct1[0, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct1[1, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct1[2, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct1[3, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct1[4, 0].set_ylabel('Fe-O / A', fontsize=11)
fig_struct1.tight_layout()
fig_struct1.subplots_adjust(hspace=0)
fig_struct1.savefig('{}/fe_structure_subplot.png'.format(folder_save_2), dpi=300, bbbox_inches='tight')

# Plot metric
rows, cols = 5, 2
run_count = 0
index = np.linspace(start=1, stop=6, num=6)
fig_struct2, ax_struct2 = plt.subplots(rows, cols, sharex='col', sharey='row', figsize=(8, 9))
for row in range(rows):
    for col in range(cols):

        for i in range(len(atoms_fe)):
            ax_struct2[row, col].plot(index, np.sort(bond_lengths_time[run_count, i])[0:6], 'kx')
        ax_struct2[row, col].set_ylim([1.1, 4])
        ax_struct2[row, col].set_xlim([1 - 0.2, 6 + 0.2])
        run_count = run_count + 1
ax_struct2[4, 0].set_xlabel('Fe-O index', fontsize=11)
ax_struct2[4, 1].set_xlabel('Fe-O index', fontsize=11)
ax_struct2[0, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct2[1, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct2[2, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct2[3, 0].set_ylabel('Fe-O / A', fontsize=11)
ax_struct2[4, 0].set_ylabel('Fe-O / A', fontsize=11)
fig_struct2.tight_layout()
fig_struct2.subplots_adjust(hspace=0)
fig_struct2.savefig('{}/fe_structure_subplot2.png'.format(folder_save_2), dpi=300, bbbox_inches='tight')
