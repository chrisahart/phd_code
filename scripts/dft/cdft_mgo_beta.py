...
hab_pbe_cpmd_fit = np.polyfit(d_hab[pbe_values], np.log(hab_pbe_cpmd[pbe_values]), 1)
hab_pbe_bw_fit = np.polyfit(d_hab[pbe_values], np.log(hab_pbe_bw[pbe_values]), 1)
hab_pbe_hw_fit = np.polyfit(d_hab[pbe_values], np.log(hab_pbe_hw[pbe_values]), 1)

print('\nPBE CPMD beta', 2 * -hab_pbe_cpmd_fit[0], 2.7 + 2*hab_pbe_cpmd_fit[0])
print('PBE BW beta', 2 * -hab_pbe_bw_fit[0], 2.7 + 2*hab_pbe_bw_fit[0])
print('PBE HW beta', 2 * -hab_pbe_hw_fit[0], 2.7 + 2*hab_pbe_hw_fit[0])

hab_pbe0_cpmd_fit, covariance_cpmd = np.polyfit(d_hab, np.log(hab_pbe0_cpmd), 1, cov=True)
...
print('\nPBE0 CPMD beta', 2 * -hab_pbe0_cpmd_fit[0], 2.7 + 2*hab_pbe0_cpmd_fit[0], 'pm', np.sqrt(np.diag(covariance_cpmd)))
